...
ateam is a 44k-LOC Go CLI + small embedded web server with a generally healthy package layout (`cmd/` thin shell, `internal/{agent,runner,calldb,prompts,runtime,root,web,…}` with clear roles). Three files have accreted into catch-alls that now dominate the structural debt: `cmd/table.go` (1390 LOC), `internal/web/handlers.go` (1433 LOC), and `internal/runner/runner.go` (1281 LOC, with a 440-line `Run` method). The rest of the codebase is consistent and well-commented; remaining findings are localized duplication, parallel command boilerplate, and a planned legacy-layout removal that is gated on a migration sentinel.

Role performing the audit: `code.structure` (merged refactor-small + architecture lens). Model: claude-opus-4-7. Thinking: standard reasoning, no extended thinking enabled. No prior `code.structure` report was found under `.ateam/roles/code.structure/history/`, so this report stands alone.
...
- **Severity**: HIGH
- **Effort**: MEDIUM
- **Description**: Every page handler lives in this file alongside legacy-layout file resolvers, run-file rendering, history dispatch, sessions/code-sessions logic, and a duplicated pricing-table builder (lines 580–591). The package already has dedicated files (`history.go`, `export.go`, `markdown.go`, `server.go`), but new handlers have continued to land in `handlers.go`. The file is the single biggest source file in the project and slows every web change — readers must skim past unrelated handlers to find one. Notably, the page-data structs (`overviewData`, `reportData`, `supervisorOutputData`, `runDetailData`, `runFileData`, `historyDetailData`, `costPageData`, `sessionsPageData`, …) and their handlers are interleaved rather than colocated with their templates.
...
## 3. `runner.Run` is a 440-line function

- **Location**: `internal/runner/runner.go:195`–`runner.go:632`
- **Scope**: module
- **Severity**: MEDIUM
- **Effort**: MEDIUM
...
- **Recommendation**: Without changing public API, split into private methods on `*Runner`:
  ...
  - `assembleSummary(state, loopResult, opts) RunSummary` — fold the post-loop result/fallback logic and the streamed-text fallback.
...
- **Location**: `internal/web/handlers.go:144–183, 510–558`, `internal/web/handlers.go:704–723`, `cmd/inspect.go:213–217`, `cmd/pool_status.go:73`, `cmd/resume.go:255`, plus the `root.IsLegacyStreamFile` predicate at `internal/root/resolve.go:402`.
- **Scope**: architecture
- **Severity**: MEDIUM
- **Effort**: MEDIUM
- **Description**: There are two file-layout branches everywhere: the new `logs/<exec_id>/{stream.jsonl,prompt.md,cmd.md,stderr.out}` and the legacy `<TS>_<ACTION>_stream.jsonl` prefix layout. `internal/root/migrate_logs.go` has a sentinel-guarded migration (`logs/.layout-v2`) that runs on every DB open via `openProjectDB`/`requireProjectDB`, so any project that has been opened with the current binary is already migrated. The dual code path now adds reading cost (every web handler with file resolution branches twice) and a class of "what does this row look like" questions. Because the predicate is purely about the suffix `_stream.jsonl` in DB rows, you can also tighten guarantees: once `MigrateLogsLayout` runs successfully it rewrites those rows.
...
- **Location**: `cmd/table.go:378–392` (`buildPricingFromConfig`), `cmd/cat.go:126` (call site), `internal/web/handlers.go:579–591` (inline conversion inside `handleRunFile`).
- **Scope**: module
- **Severity**: MEDIUM
- **Effort**: SMALL
- **Description**: The conversion from `runtime.AgentPricing` (`InputPerMTok` / `CachedInputPerMTok` / `OutputPerMTok`) to `agent.PricingTable` (per-token rates) appears twice as identical logic. Any change to the pricing schema must be made in both places; the inline copy in `handlers.go` is easy to miss because it lives inside a 90-line handler.
- **Recommendation**: Move `buildPricingFromConfig` and `mergedPricingFromConfig` from `cmd/table.go` into `internal/runtime` (e.g. `runtime.AgentPricing.ToTable()` returning `(agent.PricingTable, string)`), and update both `cmd/cat.go` and `internal/web/handlers.go` to call it. Belongs in `runtime` rather than `agent` because `runtime` already imports nothing user-facing and `agent` is the consumer.
...
## 10. `cmd.applyModel` is dead after `applyModelOverrides`

- **Location**: `cmd/table.go:770–774`
- **Scope**: local
- **Severity**: LOW
- **Effort**: SMALL
- **Description**: `applyModel(r, value)` sets the runner's model unconditionally. Since `applyRunnerOverrides` always routes through `applyModelOverrides(r, cheaper, model)`, no caller of the new path uses `applyModel` directly. A grep shows `applyModel(` appears only in its own definition (the other matches are `applyModelOverrides`). It's a candidate to delete or to mark internal-only with a comment if intended for a future caller.
- **Recommendation**: Run `grep -n "applyModel(" cmd/*.go` to confirm zero call sites outside the definition, then delete the function. Keep `applyEffort` even though it's also single-call — `applyRunnerOverrides` uses it. Recommend a final `staticcheck ./...` pass to surface any other unused-helpers in the same idiom.

## 11. Two TODO-acknowledged parallel base prompts

- **Location**: `internal/prompts/prompts.go:51` (constants), `internal/prompts/prompts.go:114–119` (`resolveBaseFileForRole`)
- **Scope**: module
- **Severity**: LOW
- **Effort**: SMALL (after the A/B compares finish)
- **Description**: `NewReportBasePromptFile` and `resolveBaseFileForRole` exist to keep legacy roles on `report_base_prompt.md` and dotted-prefix roles on `new_report_base_prompt.md` during an A/B comparison. The TODO at the constant ("fix this before v1 — merge with ReportBasePromptFile once the new role set is validated") names the cleanup explicitly. This is intentional, time-bound debt; the only structural risk is that the cleanup never happens.
...
1. **Finding 5** — Extract pricing conversion into `runtime.AgentPricing.ToTable()` and update three call sites. SMALL effort, MEDIUM severity, removes a class of "remember to update the other place" bugs.
2. **Finding 10** — Delete `cmd.applyModel` if grep confirms zero external callers. SMALL effort, LOW severity but mechanical.
...
- **Biggest files** (non-test, structural debt hotspots): `internal/web/handlers.go` (1433), `cmd/table.go` (1390), `internal/runner/runner.go` (1281), `cmd/agent_config.go` (805), `internal/prompts/prompts.go` (731), `internal/agent/codex.go` (585), `internal/runtime/config.go` (561).
...
- **Log layout**: New canonical layout is `.ateam/logs/<exec_id>/{stream.jsonl,prompt.md,cmd.md,stderr.out,settings.json}` + `.ateam/runtime/<exec_id>/`. Legacy `<TS>_<ACTION>_*` layout is migrated on first DB open via `root.MigrateLogsLayout` (sentinel `.ateam/logs/.layout-v2`).
- **Entry points**: `main.go` → `cmd.Execute` → Cobra dispatch in `cmd/root.go` (29 subcommands registered).
...
- **Known intentional duplication**: `internal/prompts/prompts.go::NewReportBasePromptFile` + `resolveBaseFileForRole` are intentional A/B branching with a `TODO: fix this before v1` marker.
