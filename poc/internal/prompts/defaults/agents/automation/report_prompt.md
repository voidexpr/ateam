...
- **Are scripts well suited for agents**: do build, test and other scripts have a terse output so they don't waste tokens ? It's best to have terse versions that agents use and either a way to get more details when needed (like test failures) or separate makefile targets / scripts for human consumption
