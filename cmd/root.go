...
func init() {
	...
	rootCmd.PersistentFlags().StringVar(&sandboxDetectionFlag, "sandbox-detection", "", "true|false: auto-detect when ateam is running inside an outer sandbox (fence/firejail/Seatbelt/bwrap) and treat it as a container so the agent's inner sandbox is skipped. Built-in default false because the signals can have false positives that silently drop defense in depth — opt in when running ateam under fence/firejail. Empty = use runtime.hcl or built-in default.")
