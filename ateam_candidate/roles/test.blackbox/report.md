...
ATeam's docs describe a large CLI surface (~30 commands, dozens of documented flags) backed by ~680 Go tests, but several primary user workflows promised by `README.md` and `COMMANDS.md` have no test asserting the documented outcome. None of the prior `test.blackbox` findings (1–13) have been addressed since the previous run 9 hours ago — they all carry forward unchanged. One new finding lands this run: the `ateam update` early-return bug fixed in commit `386e62d` shipped without a regression test, exactly the pattern this role is designed to catch.

# Role

- Role: `test.blackbox`
- Model: claude-opus-4-7 (default reasoning, no extended-thinking flag)
- Methodology: read `README.md`, `COMMANDS.md`, `CONFIG.md`, `CLAUDE.md`, the per-command `Long`/`Short` cobra descriptions, recent commit messages since the prior report, and `Makefile` to enumerate documented behaviors. Listed test names (not bodies) under `cmd/` and `internal/` to find which behaviors lack an asserting test. Implementation files were consulted only to confirm the documented behavior exists in code and to name the test file that should host the new test — never to shape the recommendation itself.
- Working tree: clean at commit `6dcf9a0`.
- Prior `test.blackbox` reports: one (run 9 hours ago at commit `ccd5003`). All findings re-verified against the current tree; none addressed.
...
## 14. NEW — `ateam update` early-return regression has no protective test

- **Title**: Commit `386e62d` fixed a silent bug where `ateam update` skipped runtime.hcl/Dockerfile rewrites; no regression test was added
- **Location**: documented behavior — `COMMANDS.md:702-709` ("refreshes embedded defaults on disk, including runtime.hcl and Dockerfile"). Bug-fix commit: `386e62d` (`cmd/update.go:50-90`, `internal/runtime/config.go:521-560` adds `RuntimeDiff` + `DiffOrgDefaults`). The commit message states explicitly: *"Fix `ateam update` so it actually refreshes runtime.hcl / Dockerfile. The early-return at len(promptDiffs)==0 skipped the runtime write, so defaults/runtime.hcl never got rewritten when only HCL changed."* No test was added in `internal/runtime/config_test.go` or anywhere under `cmd/` exercising the combined prompt+runtime diff path. `grep -l "RuntimeDiff\|DiffOrgDefaults" cmd/*_test.go internal/runtime/*_test.go` returns nothing.
- **Severity**: HIGH
- **Effort**: SMALL
- **Description**: The exact failure mode the prompt for this role calls out: *"Recent commit messages claiming behavior changes ('fix: handle empty input correctly') with no regression test for the bug."* The bug shape — when `promptDiffs` is empty but `runtimeDiffs` is not, the runtime files were silently skipped — is reachable any time prompts and HCL drift independently (a normal pattern as the binary advances). Without a test pinning the new combined-diff branch, the same bug can re-emerge from a future refactor of `runUpdate` (e.g., reverting to a single early-return check). This is the highest-priority new finding because it directly maps the role's stated mandate to a concrete missing test.
- **Recommendation**: Add `cmd/update_test.go::TestUpdateRewritesRuntimeWhenOnlyRuntimeChanged` (also satisfies finding 12 partially): (a) seed an org dir under `t.TempDir()` whose `defaults/runtime.hcl` differs from the embedded copy by a single line *and* whose prompt files exactly match the embedded copies; (b) call `runUpdate` (non-`--diff`) against that org; (c) re-read `defaults/runtime.hcl` and assert it now matches `defaults/runtime.hcl` from the embedded FS. Add a parallel sub-case for `defaults/Dockerfile`. Add a third sub-case for the negative direction — only prompts changed, runtime intact — asserting prompts get rewritten and runtime is left byte-identical. Use `embed.FS` reads via the `defaults` package (already a public dependency of `internal/runtime`).

# Quick Wins

1. **Finding 14 (NEW)** — `TestUpdateRewritesRuntimeWhenOnlyRuntimeChanged` (SMALL). Pins a recently-fixed silent bug whose commit message explicitly names a regression-test gap. Single test, three sub-cases.
2. **Finding 2** — `TestParallelCommonPromptAssembly` (SMALL). Single new test, captures `--dry-run` stdout, asserts the documented `\n\n` separator format. Protects a public prompt-assembly contract.
3. **Finding 1** — `TestExecReadsStdinWhenNoArg` (SMALL). The piping behavior is described in two doc files and shown in examples, but has zero coverage. A single os.Pipe-based test in `cmd/exec_test.go` covers it.
4. **Finding 3** — `TestPrintRolesDocsRowShape` (SMALL). Asserts the 4-cell-per-row invariant against `printRolesDocs()` output. Catches the same class of bug that ccd5003 fixed for *any* future role description.
5. **Finding 5** — `TestVersionCommandPrintsAllFourLines` (SMALL). Two minutes of test code defends the documented bug-reporting format.
...
- **Documentation sources used (top-to-bottom blackbox sweep)**:
  ...
  - Recent commit messages — `git log ccd5003..HEAD` since prior report; commit `386e62d` is the source of finding 14.
- **Test layout**:
  - `cmd/*_test.go` — 24 test files for 37 cmd files. Untested cmd files: `auto_setup`, `container_cp`, `env`, `eval`, `install`, `pool_render_mpb`, `project_rename`, `projects`, `prompt`, `root`, `runs`, `serve`, `tail`, `update`, `verify`.
  - `internal/**/*_test.go` — broad coverage of prompts, runner, web, streamutil, gitutil, config. `internal/runtime/config_test.go` covers HCL parsing only; the new `DiffOrgDefaults` (added in `386e62d`) is uncovered.
...
- **Run history**: prior `test.blackbox` report (commit `ccd5003`, 9 hours ago) — none of findings 1–13 addressed; one new finding (14) added this run.
