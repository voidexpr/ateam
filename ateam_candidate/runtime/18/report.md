...
The project's dependency surface is small (10 direct Go modules) and healthy: no archived upstreams, no GitHub advisory database hits against any direct dependency, and every declared direct dependency is actually imported. The audit environment is partially blocked — `proxy.golang.org` and `api.osv.dev` are unreachable from this sandbox (TLS / allowlist) — so version-currency claims (`go list -m -u all`) and reachability-based vuln scans (`govulncheck`) cannot be verified locally and should be run in CI; per role policy, version-currency findings are NOT filed in that condition.
...
- Methodology: read `go.mod` / `go.sum`; enumerated direct deps; verified each is imported by `git grep` across `*.go`; queried `api.github.com` for repository archived-flag and `pushed_at` for each direct dep; queried the GitHub Advisory Database (`/advisories?ecosystem=go&affects=…`) for each direct dep; grepped local module cache (`~/go/pkg/mod/...@vX`) for `// Deprecated:` markers and cross-checked against actual usage in this project.
- Audit blockers: `proxy.golang.org` returns TLS `x509: OSStatus -26276` from this sandbox; `api.osv.dev` is `blocked-by-allowlist` at the egress proxy. `go install golang.org/x/vuln/cmd/govulncheck@latest`, `go list -m -u all`, and OSV reachability checks therefore could not be run.
- Prior reports: no prior `project.dependencies` report on disk for this project. No carry-forward.

# Findings

## 1. Run `govulncheck` in CI; the local sandbox cannot reach it
...
- **Severity**: LOW
- **Effort**: SMALL
- **Description**: The conservative-mode dependency audit relies on `govulncheck ./...` to confirm that known CVEs in transitive dependencies are not reached on the call graph. From this sandbox, `proxy.golang.org` (needed for `go install golang.org/x/vuln/cmd/govulncheck@latest`) and `api.osv.dev` both fail at the egress layer, so neither this report nor a future run from this sandbox can verify reachability of any CVE. The GitHub Advisory Database query (which is reachable) returned zero advisories matching any of the ten direct dependencies at the queried versions, so there is no known-public CVE on a direct dep — but transitive call-graph reachability is unverified.
...
- **Direct dependencies** (10, all imported, all upstream-active per `api.github.com`):
...
- **License check**: deferred — `go-licenses` requires network access to `proxy.golang.org` (blocked). The project distributes a single CLI binary; the dependency set is dominated by MIT / BSD / MPL-2.0 packages by reputation, with no known GPL/AGPL surface, but this should be confirmed in CI when the network is reachable.
