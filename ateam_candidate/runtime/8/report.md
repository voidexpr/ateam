...
This is the first `test.quality` run on this branch — no prior report to merge.
...
- **Location**: `cmd/all_test.go:33-55`, `cmd/cost_test.go`, `cmd/code_test.go:29-30,71-72`, `cmd/exec_test.go:69-74`, `cmd/cat_test.go`, `cmd/report_test.go`, `cmd/review_test.go`, `cmd/model_override_combined_test.go` (≥48 save/restore sites across 8 files)
- **Severity**: MEDIUM
- **Effort**: LARGE
...
- **Location**: `cmd/exec_test.go:16-30` (the helper), used in `cmd/all_test.go`, `cmd/cost_test.go`, `cmd/code_test.go`, `cmd/report_test.go`, `cmd/review_test.go`, `cmd/inspect_test.go`, `cmd/cat_test.go`, `cmd/db_lifecycle_test.go`, `cmd/secret_test.go`, `cmd/model_override_combined_test.go`, `cmd/table_test.go`, `cmd/export_test.go` (~44 call sites across 13 files)
- **Severity**: MEDIUM
- **Effort**: LARGE
...
- **Location**: `internal/eval/worktree_test.go:17-35` (and `initGitRepo`/`newTestEnv` helpers); `internal/gitutil/gitutil_test.go:22` (`TestGetProjectMeta` walks up to the actual repo using `dir + "/../.."`)
- **Severity**: LOW
- **Effort**: MEDIUM
- **Description**: These tests are documented as unit tests (no build tag, run by default `make test`) but require `git` on PATH. `gitutil_test.go:60-63` does `t.Skip` if git is missing; `eval/worktree_test.go` does not — it will hard-fail without git. `TestGetProjectMeta` also depends on the test being run from inside the actual checkout (it walks `../..` to find the repo) — not portable to tarball / detached test environments. Local-vs-CI divergence risk: a developer who runs tests in a sandbox without git will see real failures while CI passes.
...
- **Severity**: LOW
- **Effort**: SMALL
- **Description**: `TestMaskValueShort` tests lengths 0, 1, 4, 8 (all asserted to map to `"***"`); `TestMaskValueMedium` tests length 9; `TestMaskValueLong` tests length 29. The boundary between "short" and "medium" is presumably at exactly 9 chars based on the existing inputs, but the function under test is not visible here, and there is no test at exactly the boundary value where behavior is most likely to be off-by-one. Note that this is a "missing edge case on covered code" — `MaskValue` is covered — not a test-gap finding.
- **Recommendation**: Add `MaskValue` cases at the exact transition lengths (probably 8 vs 9 and whatever the medium/long cutoff is) so an off-by-one regression in the mask logic fails a test.
...
- **Key weak spots**: `cmd/` package — heavy reliance on package-level flag globals and `os.Stdout` capture (findings #3, #4).
