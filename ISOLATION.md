...
A "no isolation" option also exists (run the agent directly against the host with no sandbox) — covered briefly below; rarely the right choice.

The rest of this guide walks through each mode in order, then covers cross-cutting concerns: secrets, authentication, shared Linux agent config (used by modes ③ and ④), precheck scripts, and troubleshooting.
